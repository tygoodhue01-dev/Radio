...
export default function HomeScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [np, n, s] = await Promise.all([getNowPlayingApi(), getNewsApi(), getShowsApi()]);
      setNowPlaying(np);
      setNews(n.slice(0, 3));
      setShows(s.slice(0, 4));
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        <View style={[styles.inner, maxW ? { maxWidth: maxW, alignSelf: 'center', width: '100%' } : undefined]}>
          ...
          <View style={styles.headerRow}>
            ...
            <View style={styles.headerRight}>
              {user && (user.role === 'admin' || user.role === 'dj') && (
                <TouchableOpacity testID="admin-nav-btn" onPress={() => router.push('/admin')} style={styles.iconBtn}>
                  <Ionicons name="settings" size={20} color={Colors.secondary} />
                </TouchableOpacity>
              )}
              {!user && (
                <TouchableOpacity testID="home-login-btn" onPress={() => router.push('/(auth)/login')} style={styles.signInBtn}>
          ...
          {/* Desktop: side-by-side hero + shows */}
          {isDesktop ? (
            <View style={styles.desktopRow}>
              {/* Now Playing Hero */}
              <View style={[styles.heroCard, { flex: 2, marginRight: Spacing.md }]}>
                ...
                <View style={styles.playerControls}>
                  <TouchableOpacity testID="play-radio-button" style={styles.playBtn} onPress={() => setIsPlaying(!isPlaying)} activeOpacity={0.7}>
                    <Ionicons name={isPlaying ? 'pause' : 'play'} size={32} color={Colors.white} />
                  </TouchableOpacity>
                </View>
                <Text style={styles.streamNote}>{isPlaying ? 'Streaming Live' : 'Tap to Listen Live'}</Text>
              </View>
              ...
              <View style={{ flex: 1 }}>
                ...
                {shows.map((show) => (
                  <View key={show.show_id} style={styles.showCardCompact}>
                    <Ionicons name="mic" size={20} color={Colors.primary} />
                    <View style={{ flex: 1, marginLeft: 10 }}>
                      <Text style={styles.showName} numberOfLines={1}>{show.name}</Text>
                      <Text style={styles.showSchedule}>{show.schedule}</Text>
                    </View>
                  </View>
                ))}
              </View>
          ...
          <View style={[styles.quickActions, isDesktop && styles.quickActionsDesktop]}>
            <TouchableOpacity testID="quick-request-btn" style={[styles.quickBtn, isDesktop && styles.quickBtnDesktop]} onPress={() => router.push('/(tabs)/requests')}>
              <Ionicons name="musical-notes" size={24} color={Colors.primary} />
              <Text style={styles.quickLabel}>Request a Song</Text>
            </TouchableOpacity>
            <TouchableOpacity testID="quick-news-btn" style={[styles.quickBtn, isDesktop && styles.quickBtnDesktop]} onPress={() => router.push('/(tabs)/news')}>
              <Ionicons name="newspaper" size={24} color={Colors.secondary} />
              <Text style={styles.quickLabel}>Latest News</Text>
            </TouchableOpacity>
            <TouchableOpacity testID="quick-rewards-btn" style={[styles.quickBtn, isDesktop && styles.quickBtnDesktop]} onPress={() => router.push('/(tabs)/rewards')}>
          ...
          {news.length > 0 && (
            <View style={styles.section}>
              ...
              <View style={isDesktop ? styles.newsGrid : undefined}>
                {news.map((article) => (
                  <TouchableOpacity
                    key={article.news_id}
...
const styles = StyleSheet.create({
  ...
  signInText: { fontSize: FontSizes.sm, fontWeight: '700', color: Colors.white },
  desktopRow: { flexDirection: 'row', marginTop: Spacing.md },
  heroCard: {
    marginTop: Spacing.md, backgroundColor: Colors.surface,
    borderRadius: BorderRadius.xl, padding: Spacing.lg,
    borderWidth: 1, borderColor: 'rgba(255,0,127,0.3)', alignItems: 'center',
  },
  liveRow: { flexDirection: 'row', alignItems: 'center', marginBottom: Spacing.md },
  liveBadge: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: 'rgba(255,240,0,0.15)',
    paddingHorizontal: 10, paddingVertical: 4, borderRadius: BorderRadius.round, marginRight: Spacing.sm,
  },
  ...
  quickActionsDesktop: { justifyContent: 'center', gap: Spacing.lg },
  quickBtn: {
    alignItems: 'center', backgroundColor: Colors.surface,
    borderRadius: BorderRadius.lg, padding: Spacing.md, width: 90,
    borderWidth: 1, borderColor: Colors.border,
  },
  quickBtnDesktop: { width: 140, paddingVertical: Spacing.lg },
  quickLabel: { fontSize: FontSizes.xs, color: Colors.textSecondary, fontWeight: '600', marginTop: 6, textAlign: 'center' },
  ...
  showCard: {
    width: 160, marginRight: Spacing.md, backgroundColor: Colors.surface,
