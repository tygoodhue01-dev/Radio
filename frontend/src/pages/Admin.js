...
export default function Admin() {
  ...
  const renderNowPlaying = () => (
    <div>
      ...
      <p className="text-sm text-[#a1a1aa] mt-1 mb-6">Update what's currently playing on air.</p>
      <div className="bg-[#18181b] rounded-xl p-6 border border-[rgba(255,255,255,0.1)]">
        <Label>SONG TITLE</Label>
        <Input value={npSong} onChange={e => setNpSong(e.target.value)} placeholder="Song title" />
        <Label>ARTIST</Label>
        <Input value={npArtist} onChange={e => setNpArtist(e.target.value)} placeholder="Artist name" />
        ...
        }}><Save size={16} /> UPDATE NOW PLAYING</Btn>
      </div>
    </div>
  );
